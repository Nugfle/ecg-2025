...
void application::key_down(unsigned char key, int x, int y) {
  ...
  case 'c':
    ...
    break;
  case 't':
    context_menu_select(MA_TERRAIN);
    break;
  case 'x':
    context_menu_select(MA_CUBE_SYSTEM_SPLIT);
    break;
  case 'r':
    context_menu_select(MA_CUBES_RECURSIVE);
    break;
  case 's':
    context_menu_select(MA_TERRAIN_SOLID);
    break;
  case 'w':
    context_menu_select(MA_TERRAIN_WIREFRAME);
    break;
  case 'l':
    context_menu_select(MA_TERRAIN_LEVELS);
    break;
  case '1':
    context_menu_select(MA_EXAMPLE_PRIMITIVES);
    break;
  case '2':
    context_menu_select(MA_EXAMPLE_CPRIMITIVES);
    break;
  case '3':
    context_menu_select(MA_EXAMPLE_3D_PRIMITIVES);
    break;
  case '4':
    context_menu_select(MA_EXAMPLE_TRANS_ORDER);
    break;
  case 27:
    glutDestroyWindow(1);
    break;
